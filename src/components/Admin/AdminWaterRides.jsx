    ...
    const initialRides = [
      {
        ...
        title: "Wave Pool",
        description:
          "Splash around in our massive wave pool, perfect for all ages!",
      },
      {
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRyXSO0Dje1UrDEVH7UT5jQzEZn3ma-Tjj5EQ&s",
        title: "Lazy River",
        description:
          "Relax and float along our gentle lazy river ride, perfect for a leisurely day!",
      },
      {
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQPpCIgsk7E7o1_TL36HRy4lr7cgfOCHzwKeA&s",
        title: "Tidal Wave",
        description:
          "Experience the thrill of our giant swing that sends you plummeting into the splash zone!",
      },
    ];
  ...
  return (
    <div className="p-5">
      ...
      <form onSubmit={handleAddOrUpdateRide} className="mb-4">
        ...
        <label htmlFor="img" className="block mb-1 text-gray-700">
          Image Link
        </label>
        <input
          id="img"
      ...
      <h2 className="text-xl mb-2 text-gray-900">Current Water Rides</h2>
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {rides.map((ride) => (
          ...
          >
            ...
            />
            <div className="p-5">
              <h1 className="font-bold text-xl text-gray-900">{ride.title}</h1>
              <p className="text-gray-700">{ride.description}</p>
              <div className="flex justify-between mt-4">
                <button
                  onClick={() => handleEditRide(ride)}
                  className="w-1/2 p-2 rounded-lg bg-yellow-700 text-white font-semibold hover:bg-yellow-800 transition duration-200 ease-in-out mr-1"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDeleteRide(ride.id)}
                  className="w-1/2 p-2 rounded-lg bg-red-700 text-white font-semibold hover:bg-red-800 transition duration-200 ease-in-out"
                >
                  Delete
                </button>
              </div>
